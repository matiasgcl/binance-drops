...
sheet1.cell(row=2, column=3, value='Time Low (Close)').alignment = Alignment(horizontal="center")
sheet1.cell(row=2, column=4, value='High').alignment = Alignment(horizontal="center")
sheet1.cell(row=2, column=5, value='Time High (Open)').alignment = Alignment(horizontal="center")
sheet1.cell(row=2, column=6, value='Price Drop').alignment = Alignment(horizontal="center")
sheet1.cell(row=2, column=7, value='T high - T low').alignment = Alignment(horizontal="center")
data = []
perc = []
for i in range(len(result)):
    ...
    if(len(klines)>0):
        ...
        timelow = closetimes[indexlow]
        timelow_datetimeFormat = timelow/86400000+25569

        highsaux = highs[:indexlow+1]
        opentimesaux = opentimes[:indexlow+1]
        high = (max(highsaux))
        indexhigh = highsaux.index(high)
        timehigh = opentimesaux[indexhigh]
        timehigh_datetimeFormat = timehigh/86400000+25569

        var_perc = (float(high)-float(low))/float(high)
        timevar = (-1)*(timehigh-timelow)/(1000*86400)
...
sheet2.cell(row=2, column=3, value='Time Low (Close)').alignment = Alignment(horizontal="center")
sheet2.cell(row=2, column=4, value='High').alignment = Alignment(horizontal="center")
sheet2.cell(row=2, column=5, value='Time High (Open)').alignment = Alignment(horizontal="center")
sheet2.cell(row=2, column=6, value='Price Drop').alignment = Alignment(horizontal="center")
sheet2.cell(row=2, column=7, value='T high - T low').alignment = Alignment(horizontal="center")
...
saveas = 'Drops-Top'+str(len(result))+'Coins-'+granularity+'-'+init_date+'-to-'+end_date+'.xls'
